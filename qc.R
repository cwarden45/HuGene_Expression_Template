...
call.rate.file = as.character(param.table$Value[param.table$Parameter == "Call_Rate_Table"])
...
normalized.table = read.table(RMA.file, sep="\t", header=T)
clusterID = as.character(normalized.table$transcript.cluster)
normalized.mat = normalized.table[,match(sampleID, names(normalized.table))]
...
write.table(quantiles, "RMA_quantiles.txt",sep="\t", quote=F, row.names=F)
...
pca.text.file = "pca.txt"
write.table(pca.table, pca.text.file, quote=F, row.names=F, sep="\t")

call.table = read.table(call.rate.file,head=T, sep="\t")
call.table = call.table[match(sampleID, call.table$sample),]
...
updated.sample.description = gsub(".txt$","_with_call_rate_info.txt",sample.description.file)
output.table = data.frame(sample.table, call.rate=call.rate, call.rate.group = call.rate.group)
write.table(output.table, updated.sample.description, quote=F, sep="\t", row.names=F)
...
for (i in 1:length(plot.groups)){
	...
	clusterID = sampleID[!is.na(sample.table[,group])]
	...
	png(paste("call_rate_vs_PC1_by",group,".png",sep=""))
	plot(call.rate, pc.values$PC1, xlab="Call Rate", ylab="PC1", pch=16, col=labelColors)
	dev.off()
	...
	cluster.order.file = paste("cluster_by_",group,".txt",sep="")
			...
			if(j == 1)
				{
					...
					expr <- den$x
					freq <- den$y
					plot(expr, freq, type="l", xlab = paste("log2(RMA)",sep=""), ylab = "Density",
							xlim=c(expr.min,expr.max), ylim=c(0,0.5), col=labelColors[j], main=paste("Color by ",group,sep=""))
					if(group.type == "continuous"){
						legend("right",legend=c(round(plot.var.max,digits=1),rep("",length(color.range)-2),round(plot.var.min,digits=1)),
